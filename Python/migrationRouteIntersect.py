...
def routeIntersect(route):
    ...
    query = "SELECT A.MIGRATION_ID, B.MIGRATION_ID FROM S1234874.MIGRATION A, S1234874.MIGRATION B WHERE A.MIGRATION_ID = (SELECT MIGRATION_ID FROM S1217815.DRAGONS WHERE NAME = '"+dragonroute+"') AND SDO_RELATE(A.ROUTE, B.ROUTE, 'MASK = TOUCH') = 'TRUE'" 
