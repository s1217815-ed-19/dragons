...
SELECT A.NAME, B.FOOD_TYPE, SDO_GEOM.SDO_DISTANCE(A.LOCATION, B.LOCATION, .005)
FROM S1234874.SETTLEMENTS A, S1234874.FOOD_SOURCE B
WHERE A.SETTLEMENT_ID = 10 AND B.FOOD_SOURCE_ID = 3;


--getting centroids of all the polygon regions
SELECT A.REGION_ID, SDO_GEOM.SDO_CENTROID(A.SHAPE,0.005).SDO_POINT.X,
SDO_GEOM.SDO_CENTROID(A.SHAPE,0.005).SDO_POINT.Y
FROM S1234874.REGION A;
